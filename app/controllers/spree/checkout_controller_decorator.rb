Spree::CheckoutController.class_eval do
  ...
  def set_addresses
    ...
    if params[:order][:ship_address_id].to_i > 0
      ...
      Spree::Address.find(params[:order][:ship_address_id]).user_id != spree_current_user.id && 
        raise("Frontend address forging")
    else
    ...
    if params[:order][:bill_address_id].to_i > 0
      ...
      Spree::Address.find(params[:order][:bill_address_id]).user_id != spree_current_user.id && 
        raise("Frontend address forging")
    else
